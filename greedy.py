...
def SD_move(layout, pos=0,  stack=-1):
    ...
    while len(layout.stacks[s_o])>0:
        ev, s_o = actions[pos]
        s_d, _ = layout.select_destination_stack(s_o)
        layout.move(s_o,s_d)
        if layout.reachable_height(s_o)==layout.H: return True, len(actions)
...
def fill_stack(layout, s_d, n, ori, rank):
    for i in range(n):
        s_o = layout.select_origin_stack(s_d, ori, rank)
...
def is_good_stack(stack):
    return not is_bad_stack(stack)

def is_bg_action(stacks, action):
    s_o, s_d = action

    if is_bad_stack(stacks[s_o]) and is_good_stack(stacks[s_d]):
        if gvalue(stacks[s_o]) <= gvalue(stacks[s_d]):
            return True
    return False

def is_gg_action(stacks, action):
    s_o, s_d = action

    if is_good_stack(stacks[s_o]) and is_good_stack(stacks[s_d]):
        return True
    return False

def get_action_type(stacks, action):
    if is_bg_action(stacks, action):
        return "BG"
    elif is_gg_action(stacks, action):
        return "GG"
    else:
        return "XX"
